...
let handler = async (m, { conn }) => {
    ...
    if (new Date - user.lastweekly > 604800000) {
        conn.reply(m.chat, `Anda sudah mengklaim dan mendapatkan 12000 💵money dan 1 🎁Legendary crate`, m)
        user.money += 12000
        user.legendary += 1
...
function button(teks, user) {
    ...
    if (claim) buttons.push({buttonId: `.daily`, buttonText: {displayText: 'daily'}, type: 1})
    if (buttons.length == 0) throw teks
    
    const buttonMessage = {
        contentText: teks,
        footerText: '©Raiden-Bot',
        buttons: buttons,
        headerType: 1
    }
