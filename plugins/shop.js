...
Mora Crystal: ${BMoraCrystal}
Sampah:     ${Bsampah}
Armor:       ${armor}
fishingrod: ${Bfishingrod}\n\n
*Barang   | Harga Jual*\n
Potion:       ${Spotion}
Diamond:     ${Sdiamond}
Common:     ${Scommon}
...
Mora Crystal: ${SMoraCrystal}
fishingrod: ${Sfishingrod}
Sampah:     ${Ssampah}\n\n
`.trim()
    try {
            ...
            case 'buy':
                switch (_type) {
                    ...
                    case 'legendary':
                        ...
                        break
                    case 'moracrystal':
                            if (global.db.data.users[m.sender].money >= BMoraCrystal * count) {
                                global.db.data.users[m.sender].MoraCrystal += count * 1
                                global.db.data.users[m.sender].money -= BMoraCrystal * count
                                conn.reply(m.chat, `Succes membeli ${count} Mora Crystal dengan harga ${BMoraCrystal * count} money`, m)
                            } else conn.reply(m.chat, `Uang anda tidak cukup untuk membeli ${count} Mora Crystal dengan harga ${BMoraCrystal * count} money*`, m)
                    ...
                    case 'armor':
                        ...
                        case 'fishingrod':
                            if (global.db.data.users[m.sender].money >= Bfishingrod * count) {
                                ...
                                conn.reply(m.chat, `Succes membeli ${count} ${_type} dengan harga ${Bfishingrod * count} money`, m)
                            } else conn.reply(m.chat, `Uang anda tidak cukup untuk membeli ${count} ${_type} dengan harga ${Bfishingrod * count} money`.trim(), m)
            ...
            case 'sell': 
                switch (_type) {
                    ...
                    case 'diamond':
                        ...
                        break
                    case 'moracrystal':
                        if (global.db.data.users[m.sender].MoraCrystal >= count * 1) {
                            global.db.data.users[m.sender].money += SMoraCrystal * count
                            global.db.data.users[m.sender].MoraCrystal -= count * 1
                            conn.reply(m.chat, `Succes menjual ${count} Mora Crystal dengan harga ${SMoraCrystal * count} money`.trim(), m)
                        } else conn.reply(m.chat, `Mora Crystal kamu tidak cukup`.trim(), m)
                        break
                    default:
                        return conn.reply(m.chat, Kchat, m)
                }
