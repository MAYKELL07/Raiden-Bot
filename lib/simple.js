...
import tr from 'translate-google'
const language = global.language || 'en'
...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    const oldRelay = conn.relayMessage
    /**
     * @param  {} jid
     * @param  {} message
     * @param  {} options
    ...
    const mtype = Object.keys(message)
    const m = message[mtype]
    await tr(m.text || m.caption, {from: 'auto', to: language}).then(translateText => {
    message[mtype].text = translateText
    message[mtype].caption = translateText})
    return oldRelay(jid, message,  options)
    }
...
END:VCARD
        ...
        reply: {
            ...
            async value(jid, text = '', quoted, options) {
                ...
                if (typeof text == 'string') {
                    text = await tr(text || quoted, {from: 'auto', to: language})
        ...
        sendHydrated: {
            ...
            async value(jid, text = '', footer = '', buffer, url, urlText, call, callText, buttons, quoted, options) {
                let type
                if (typeof text === 'string') text = await tr(text || footer, {from: 'auto', to: language})
